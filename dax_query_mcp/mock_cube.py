...
class MockRecordset:
    ...
    closed: bool = False
    _index: int = 0
    _field_objects: list[MockField] = field(default_factory=list)

    def __post_init__(self) -> None:
        self._field_objects = [MockField(name) for name in self.fields]
        self._sync_field_values()

    @property
    def Fields(self) -> list[MockField]:
        return self._field_objects
    ...
    def _sync_field_values(self) -> None:
        if not self.EOF:
            for i, field_obj in enumerate(self._field_objects):
                field_obj.Value = self.rows[self._index][i]
